...
class _DashboardScreenState extends State<DashboardScreen>
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _allRecords = List<Map<String, dynamic>>.from(records);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoading
          ? const Center(
          child:
          CircularProgressIndicator(color: Color(0xFF2D7A4F)))
          : FadeTransition(
        opacity: _fadeAnim,
  ...
  Widget _buildAnalyticsSection() {
    ...
    final maxVal =
    _weeklyBarData.reduce((a, b) => a > b ? a : b).clamp(0.1, double.infinity);
  ...
  Widget _buildMonthlyGoal() {
    final progress =
    (_monthlyWeight / _monthlyGoal).clamp(0.0, 1.0);
    final remaining =
    (_monthlyGoal - _monthlyWeight).clamp(0, double.infinity);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            '${_monthlyWeight.toStringAsFixed(1)} / ${_monthlyGoal.toStringAsFixed(0)} kg recycled  ·  ${remaining.toStringAsFixed(1)} kg remaining',
            style: GoogleFonts.dmSans(
      ...
      child: Column(
        ...
        children: [
          ...
          ..._categoryBreakdown.entries.map((e) {
            ...
            final icon =
                catIcons[e.key] ?? Icons.recycling_rounded;
            return Padding(
        ...
        Container(
          ...
          child: Column(
            children: _recentActivity
              ...
              final points =
                  (r['points'] as num?)?.toInt() ?? 0;
              final date =
              DateTime.parse(r['date'] as String);
              final dateStr =
                  '${date.day.toString().padLeft(2, '0')}/${date.month.toString().padLeft(2, '0')}';
              final color =
                  catColors[cat] ?? const Color(0xFF3DAB6A);
              final bg =
                  catBg[cat] ?? const Color(0xFFDFF5E9);
              final icon =
                  catIcons[cat] ?? Icons.recycling_rounded;
              final isLast =
                  i == _recentActivity.length - 1;
